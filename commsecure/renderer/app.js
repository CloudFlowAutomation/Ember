...
(function () {
  ...
  // (currently only {t:'image'}); anything else is a plain text message.
  // Typed text is scrubbed of the byte so the two can never collide.
  ...
  let socket = null;
  let session = null;   // ephemeral X25519 keypair for this connection
  ...
  function addMessage({ id, from, author, text, image, own, system, warning }) {
  ...
  function deleteForEveryone(id) {
    ...
    sendEncrypted(ENVELOPE_PREFIX + JSON.stringify({ t: 'delete', id }));
    tombstoneMessage(el);
  }
  ...
  function handleRoster(roster) {
    ...
    for (const entry of roster) {
      ...
      // Authenticate the session key before trusting it: the identity key
      // must have signed it. A relay swapping session keys fails here.
      if (!csc.verifySessionKey(entry.idpk, entry.pubkey, entry.sig)) {
        ...
        addMessage({
          system: true,
          warning: true,
          text: `Warning: "${entry.username}" presented a session key its identity did not sign — possible MITM. They are excluded from your messages.`,
        });
        continue;
      }
  ...
  // ---- Private rooms (lobby + per-room Lambda MicroVMs) ----

  // The lobby is the control plane that provisions each room's MicroVM.
  // Rooms use the default lobby unless the user sets their own rooms API
  // URL in Settings (persisted across sessions).
  ...
  function setMode(next) {
    ...
    directFields.classList.toggle('hidden', next !== 'direct');
  ...
  async function lobbyFetch(base, path, body) {
    let res;
    try {
  ...
  createRoomBtn.addEventListener('click', async () => {
    ...
    try {
      const room = await lobbyFetch(lobbyUrl(), '/rooms', { duration_minutes: minutes });
  ...
  function openSocket(url, extraOpts) {
    ...
    socket.on('connect', () => {
      // Fresh ephemeral keypair per connection (forward secrecy), signed by
      // the persistent identity key (authenticity).
      session = csc.newSession();
      ...
      addMessage({
        system: true,
        text: 'Connected. Messages are end-to-end encrypted with perfect forward secrecy; session keys are signed by persistent identities.',
      });
    });
    ...
    socket.on('e2e_message', (msg) => {
      ...
      const plaintext = csc.decrypt(peer.state, msg);
    ...
    socket.on('room_closed', (info) => {
      const reason = info && info.reason === 'idle'
  ...
  // peer and fan it out through the relay.
  function sendEncrypted(plaintext) {
    const recipients = [];
    for (const [sid, peer] of peers) {
      if (!peer.state) continue;
      const sealed = csc.encrypt(peer.state, plaintext);
      recipients.push({ to: sid, n: sealed.n, nonce: sealed.nonce, ct: sealed.ct });
    }
  ...
  composer.addEventListener('submit', (e) => {
    ...
    sendEncrypted(ENVELOPE_PREFIX + JSON.stringify({ t: 'text', id, text }));
    addMessage({ id, author: username, text, own: true });
  });
  ...
  async function sendImageFile(file) {
    ...
    try {
      ...
      const id = newMessageId();
      sendEncrypted(ENVELOPE_PREFIX + JSON.stringify({ t: 'image', id, mime, data }));
      addMessage({ id, author: username, image: dataUrl, own: true });
    } catch (e) {
      addMessage({
        system: true,
        warning: true,
